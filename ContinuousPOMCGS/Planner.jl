...
mutable struct ContinuousPlannerPOMCGS
	...
	_max_planning_secs::Int64                    # default 1e6
...
function NormalizeDict(d::Dict{Any, Float64})
    sum = 0.0
    for (key, value) in d
        sum += value
    end
...
function GetValueQMDP(b::Dict{Any, Float64}, actions, Q_learning_policy::Qlearning)
    max_value = typemin(Float64)
...
function GetValueQMDP(b::Dict{Any, Float64}, Q_learning_policy::Qlearning)
    value = 0.0
...
function CollectSamplesAndBuildNewBeliefs(bool_state_grid, 
    ...
    all_dict_weighted_samples = Dict{Int64, Dict{Any,Float64}}()
    sum_R_a = 0.0
    sum_all_weights = 0.0
    ...
    num_threads = Threads.nthreads()
    all_dict_weighted_samples_threads = Vector{Dict{Int64, Dict{Any,Float64}}}()
    ...
    for i in 1:num_threads
        push!(all_dict_weighted_samples_threads, Dict{Int64,Dict{Any,Float64}}())
    ...
    Threads.@threads for i in 1:length(collected_obs_vec)
        ...
        if haskey(all_dict_weighted_samples_threads[id_thread], abstract_obsI)
            ...
            all_dict_weighted_samples_threads[id_thread][abstract_obsI] = Dict{Any,Float64}()
            all_oI_weight_threads[id_thread][abstract_obsI] = w 
            all_dict_weighted_samples_threads[id_thread][abstract_obsI][sp] = w
        end
    end
    ...
    for i in 1:current_nb_abstract_obs
        all_dict_weighted_samples[i] = Dict{Int64,Float64}()
        all_oI_weight[i] = 0.0
    end
...
function Simulate(pomdp,
	...
	if fsc._nodes[nI]._visits_action[a] == 1
		return ProcessActionWeightedParticle(pomdp, fsc, nI, a, nb_process_action_samples, discount, Q_learning_policy)
	end
	...
	sum_R_n_a += r
	fsc._nodes[nI]._R_action[a] = sum_R_n_a / (nb_process_samples + fsc._nodes[nI]._visits_action[a] + 1)
	esti_V = fsc._nodes[nI]._R_action[a] + discount * Simulate(pomdp, fsc, bool_state_grid, state_grid, bool_APW, sp, nI_next, depth + 1, nb_process_samples, discount, c, k_a, alpha_a, C_star, epsilon, Q_learning_policy)
	fsc._nodes[nI]._Q_action[a] = fsc._nodes[nI]._Q_action[a] + ((esti_V - fsc._nodes[nI]._Q_action[a]) / fsc._nodes[nI]._visits_action[a])
	fsc._nodes[nI]._V_node = esti_V
...
function CollectSamplesAndBuildNewBeliefsWeightedParticles(pomdp, fsc::FSC, nI::Int64, a, nb_process_action_samples::Int64)
	...
	# Prepare data for multi-thread computation
	num_threads = Threads.nthreads()
	all_dict_weighted_samples_threads = [Dict{Int64, Dict{Any, Float64}}() for _ in 1:num_threads]
	...
	Threads.@threads for i in 1:length(all_keys)
		...
		@inbounds for _ in 1:nb_sim
			...
			if !haskey(dict_thread, o)
				dict_thread[o] = Dict{Any, Float64}()
				weight_thread[o] = 0.0
			end
	...
	@inbounds for id_thread in 1:num_threads
		@inbounds for o in keys(all_dict_weighted_samples_threads[id_thread])
			if !haskey(all_dict_weighted_samples, o)
				all_dict_weighted_samples[o] = Dict{Any, Float64}()
				all_oI_weight[o] = 0.0
			end
		end
	end
	...
	@inbounds for id_thread in 1:num_threads
		@inbounds for o in keys(all_dict_weighted_samples_threads[id_thread])
			...
			@inbounds for (key, value) in local_dict
...
function EvaluateUpperBound(b0, pomdp, fsc::FSC, Q_learning_policy::Qlearning, discount::Float64, nb_sim::Int64, C_star::Int64)
	...
	function simulate_once()
		...
		while step <= max_steps && discount_powers[step+1] > discount_threshold && !isterminal(pomdp, s)
			a = GetBestAction(fsc._nodes[nI])
			if a == -1
				sum_r_sim_i += discount_powers[step+1] * GetValueQMDP(fsc._nodes[nI]._dict_weighted_samples, Q_learning_policy)
				break
			end
