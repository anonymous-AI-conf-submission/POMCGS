...
mutable struct ContinuousPlannerPOMCGS
	...
	ContinuousPlannerPOMCGS(max_min_r, Q_learning_policy::Qlearning) = new(
		10000,
		0.05,
		1e6,
		1e7,
		...
		0.01,
		Q_learning_policy,
		LogResult(Vector{Int64}(), Vector{Float64}(), Vector{Float64}(), Vector{Float64}(), Vector{Float64}(), Vector{Int64}()),
		50,
		2.0,
		0.2,
		false,
...
function Simulate(pomdp,
	...
	esti_V =
		fsc._nodes[nI]._R_action[a] +
		discount * Simulate(pomdp, fsc, sp, nI_next, depth + 1, bool_state_grid, state_grid, bool_discrete_obs, bool_PCA, PCA_dim, nb_process_samples, discount, c, epsilon, k_a, alpha_a, C_star, bool_APW, Q_learning_policy, nb_abstract_obs)
...
function CollectSamplesAndBuildNewBeliefsWeightedParticles(pomdp, fsc::FSC, nI::Int64, a, nb_process_action_samples::Int64)
	...
	num_threads = Threads.nthreads()
	all_dict_weighted_samples_threads = [Dict{Int64, OrderedDict{Any, Float64}}() for _ in 1:num_threads]
	all_oI_weight_threads = [Dict{Int64, Float64}() for _ in 1:num_threads]
	...
	Threads.@threads for i in 1:length(all_keys)
		...
		nb_sim = ceil(Int, w * nb_process_action_samples)

		# Use local variables for thread data to reduce repeated lookups
		dict_thread = all_dict_weighted_samples_threads[id_thread]
		weight_thread = all_oI_weight_threads[id_thread]

		for _ in 1:nb_sim
			sp, o, r = @gen(:sp, :o, :r)(pomdp, s, a)
			sum_R_a_threads[id_thread] += r
			sum_all_weights_threads[id_thread] += 1.0

			if !haskey(dict_thread, o)
				dict_thread[o] = OrderedDict{Any, Float64}()
				weight_thread[o] = 0.0
			end

			weight_thread[o] += 1.0

			if haskey(dict_thread[o], sp)
				dict_thread[o][sp] += 1.0
			else
				dict_thread[o][sp] = 1.0
			end
		end
	...
	# Merge threads data
	sum_R_a = sum(sum_R_a_threads) / sum(sum_all_weights_threads)
	sum_all_weights = sum(sum_all_weights_threads)

	# Initialize all_dict_weighted_samples and all_oI_weight
	for id_thread in 1:num_threads
		for o in keys(all_dict_weighted_samples_threads[id_thread])
			if !haskey(all_dict_weighted_samples, o)
				all_dict_weighted_samples[o] = OrderedDict{Any, Float64}()
				all_oI_weight[o] = 0.0
			end
		end
	end
...
function CollectSamplesAndBuildNewBeliefs(bool_state_grid,
	...
	# Preallocating main dictionaries
	all_oI_weight = Dict{Int64, Float64}()
	all_dict_weighted_samples = Dict{Int64, OrderedDict{Any, Float64}}()
	...
	all_dict_weighted_samples_threads = [Dict{Int64, OrderedDict{Any, Float64}}() for _ in 1:num_threads]
	all_oI_weight_threads = [Dict{Int64, Float64}() for _ in 1:num_threads]
	collected_obs_vec_threads = [Vector{Vector{Float64}}() for _ in 1:num_threads]
	collected_sp_w_pairs_threads = [Vector{Pair{Any, Float64}}() for _ in 1:num_threads]
	map_discrete2continuous_states_threads = [Dict{Vector{Float64}, Vector{Any}}() for _ in 1:num_threads]

	# Using keys directly from dictionary
	# all_keys = keys(fsc._nodes[nI]._dict_weighted_samples)
    all_keys = collect(keys(fsc._nodes[nI]._dict_weighted_samples))
	Threads.@threads for i in 1:length(all_keys)
		id_thread = Threads.threadid()
        s = all_keys[i]
        w = fsc._nodes[nI]._dict_weighted_samples[s]
		nb_sim = ceil(Int, w * nb_samples)  # Convert to Int to avoid warnings
		w = 1.0
		for _ in 1:nb_sim
			sp, o, r = Step(fsc, pomdp, s, a, bool_state_grid, state_grid, id_thread, map_discrete2continuous_states_threads)
			o_vec = convert_o(Vector{Float64}, o, pomdp)
	...
	# Merge data after threading
	sum_R_a = sum(sum_R_a_threads) / sum(sum_all_weights_threads)
	collected_obs_vec = reduce(vcat, collected_obs_vec_threads)
	collected_sp_w_pairs = reduce(vcat, collected_sp_w_pairs_threads)

	if bool_state_grid
		for map_thread in map_discrete2continuous_states_threads
			merge!(fsc._map_discrete2continuous_states, map_thread)
		end
	end

	# Reduce dimensionality with PCA if required
	if bool_PCA
		X = reduce(hcat, collected_obs_vec)
		PCA_M = fit(PCA, X; maxoutdim = PCA_dim)
		X = predict(PCA_M, X)
	...
	# Clustering observations with KMeans
	X = reduce(hcat, collected_obs_vec)
	kmeans_result = kmeans(X, nb_abstract_obs; maxiter = 30)
    mu = kmeans_result.centers

    # Convert SubArray to Vector before storing in the set
    set_abstract_obs = Set{Vector{Float64}}()
    for i in 1:nb_abstract_obs
        push!(set_abstract_obs, vec(mu[:,i]))
    end

	# Store abstract observations
	StoreAllAbstractObs(fsc, nI, a, set_abstract_obs)

	# Updating belief with samples
	Threads.@threads for i in 1:length(collected_obs_vec)
		id_thread = Threads.threadid()
		abstract_obsI = FindMostCloseAbstractObs(collected_obs_vec[i], fsc._nodes[nI]._abstract_observations[a])
		sp_w_pair = collected_sp_w_pairs[i]
		sp, w = sp_w_pair.first, sp_w_pair.second

		dict_thread = all_dict_weighted_samples_threads[id_thread]
		weight_thread = all_oI_weight_threads[id_thread]

		if haskey(dict_thread, abstract_obsI)
			weight_thread[abstract_obsI] += w
			dict_thread[abstract_obsI][sp] = get(dict_thread[abstract_obsI], sp, 0.0) + w
		else
			dict_thread[abstract_obsI] = OrderedDict{Any, Float64}(sp => w)
			weight_thread[abstract_obsI] = w
		end
	end

	# Merge thread results into main dictionaries
	current_nb_abstract_obs = length(fsc._nodes[nI]._abstract_observations[a])
	for abs_oI in 1:current_nb_abstract_obs
		all_dict_weighted_samples[abs_oI] = OrderedDict{Int64, Float64}()
		all_oI_weight[abs_oI] = 0.0
	end
	...
	return sum_R_a, sum(sum_all_weights_threads), all_oI_weight, all_dict_weighted_samples
end
...
function EvaluationWithSimulationFSC(b0, pomdp, fsc::FSC, discount::Float64, nb_sim::Int64, vec_evaluation_value, vec_valid_value, vec_unvalid_rate)
	...
	discount_powers = [discount^i for i in 0:nb_sim]

	# Initialize thread-local accumulators
	sum_r_threads = zeros(Float64, Threads.nthreads())
	sum_r_valid_threads = zeros(Float64, Threads.nthreads())
	sum_unvalid_search_threads = zeros(Int64, Threads.nthreads())
	...
	function simulate_once()
		...
		while discount_powers[step+1] > discount_threshold && !isterminal(pomdp, s)
			if bool_random_pi && !bool_sim_i_invalid
				bool_sim_i_invalid = true
				return 0.0, bool_sim_i_invalid  # Early exit for invalid simulation
			end

			if nI == -1 || fsc._nodes[nI]._visits_node == 0
				bool_random_pi = true
			end

			a = bool_random_pi ? a_safe : GetBestAction(fsc._nodes[nI])

			sp, o, r = @gen(:sp, :o, :r)(pomdp, s, a)
			s = sp
			sum_r_sim_i += discount_powers[step+1] * r

			eta_nI = fsc._eta[nI]
			if haskey(eta_nI, Pair(a, o))
				nI = eta_nI[Pair(a, o)]
			else
				bool_random_pi = true
			end
		...
		return sum_r_sim_i, bool_sim_i_invalid
	end

	Threads.@threads for sim_i in 1:nb_sim
		id_thread = Threads.threadid()
		sum_r_sim_i, bool_sim_i_invalid = simulate_once()

		sum_r_threads[id_thread] += sum_r_sim_i
		if !bool_sim_i_invalid
			sum_r_valid_threads[id_thread] += sum_r_sim_i
		else
			sum_unvalid_search_threads[id_thread] += 1
		end
	end
	...
	sum_r = sum(sum_r_threads)
	sum_r_valid = sum(sum_r_valid_threads)
	sum_unvalid_search = sum(sum_unvalid_search_threads)

	valid_simulations = nb_sim - sum_unvalid_search
	push!(vec_evaluation_value, sum_r / nb_sim)
	push!(vec_valid_value, valid_simulations > 0 ? sum_r_valid / valid_simulations : 0.0)
	push!(vec_unvalid_rate, nb_sim > 0 ? sum_unvalid_search / nb_sim : 0.0)
...
function EvaluateUpperBound(b0, pomdp, fsc::FSC, Q_learning_policy::Qlearning, discount::Float64, nb_sim::Int64, C_star::Int64)
	...
	# Precompute discount factors
	discount_threshold = 0.01
	max_steps = ceil(Int, log(discount_threshold) / log(discount))
	discount_powers = [discount^i for i in 0:max_steps]
	...
	function simulate_once()
		...
		while step <= max_steps && discount_powers[step+1] > discount_threshold && !isterminal(pomdp, s)
			a = GetBestAction(fsc._nodes[nI])
			if a == -1
				sum_r_sim_i += discount_powers[step+1] * GetValueQMDP(fsc._nodes[nI], Q_learning_policy)
				break
			end
	...
	Threads.@threads for sim_i in 1:nb_sim
		id_thread = Threads.threadid()
		sum_r_threads[id_thread] += simulate_once()
	end
